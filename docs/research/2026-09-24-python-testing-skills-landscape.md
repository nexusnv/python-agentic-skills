...
The evidence supplies six design considerations to evaluate for the two skills:
...
These are the research snapshot's secondary GitHub values, not skill-specific adoption data. **Observation timestamp: 2026-09-24 UTC.** The interval is 2026-08-24 through 2026-09-24; repository pages are mutable, and no archived metrics response is included. The numbers are manually transcribed observations from that dated research run, not live facts or values that can be reconstructed from the current API later.

The collection definitions and equivalent `gh api` command patterns were:

- **Stars and forks:** repository metadata from `GET /repos/OWNER/REPO`; equivalently, `gh api repos/OWNER/REPO --jq '[.stargazers_count, .forks_count]'`.
- **Contributors:** the count returned by the contributors endpoint, `GET /repos/OWNER/REPO/contributors`; equivalently, `gh api --paginate repos/OWNER/REPO/contributors --jq '.[].login' | sort -u | wc -l`.
- **Releases:** the count returned by the releases endpoint, `GET /repos/OWNER/REPO/releases`; equivalently, `gh api --paginate repos/OWNER/REPO/releases --jq '.[].id' | wc -l`.
- **30-day commits:** the search-commit count for the date window, `GET /search/commits?q=repo:OWNER/REPO+committer-date:2026-08-24..2026-09-24`; equivalently, `gh api -X GET search/commits -f q='repo:OWNER/REPO committer-date:2026-08-24..2026-09-24' --jq '.total_count'`.
- **Default-branch history:** the count of all pages returned by `GET /repos/OWNER/REPO/commits?per_page=100`, the repository's default branch; equivalently, `gh api --paginate 'repos/OWNER/REPO/commits?per_page=100' --jq '.[].sha' | wc -l`.

The exact historical responses were not archived, so these commands document the measurement procedure rather than provide a later historical replay.
...
## 4. Design patterns observed in selected popular skills

These are selected exemplars, not a controlled comparison. The table records source-observable design patterns and explicit design inferences; it does not establish that a pattern produced adoption, that a skill is high quality, or that one skill outperforms another.
...
| Sharp triggers | The specification asks descriptions to include what and when; `tdd`, `diagnosing-bugs`, `systematic-debugging`, and `verification-before-completion` use explicit phrases such as “test-first,” “broken/failing/slow,” “before proposing fixes,” and “before claiming” ([`tdd` source](https://github.com/mattpocock/skills/blob/c55ee46073ed923f86ce59a5eb3b6d895095d1b7/skills/engineering/tdd/SKILL.md), [`diagnosing-bugs` source](https://github.com/mattpocock/skills/blob/c55ee46073ed923f86ce59a5eb3b6d895095d1b7/skills/engineering/diagnosing-bugs/SKILL.md), [systematic-debugging source](https://github.com/obra/superpowers/blob/5bf4e78011075bcfc0dc295f0724994cd123ee71/skills/systematic-debugging/SKILL.md), [verification source](https://github.com/obra/superpowers/blob/5bf4e78011075bcfc0dc295f0724994cd123ee71/skills/verification-before-completion/SKILL.md)). | Give each testing skill a non-overlapping trigger vocabulary: examples/parameterization versus generated properties, plus a shared boundary to verification. |
...
| Hard gates | systematic-debugging says no fix without root-cause investigation; verification-before-completion requires identify → run → read → verify → claim; `tdd` requires a failing test before green ([`tdd` source](https://github.com/mattpocock/skills/blob/c55ee46073ed923f86ce59a5eb3b6d895095d1b7/skills/engineering/tdd/SKILL.md), [systematic-debugging source](https://github.com/obra/superpowers/blob/5bf4e78011075bcfc0dc295f0724994cd123ee71/skills/systematic-debugging/SKILL.md), [verification source](https://github.com/obra/superpowers/blob/5bf4e78011075bcfc0dc295f0724994cd123ee71/skills/verification-before-completion/SKILL.md)). | Gate completion on observed red/green evidence and report unmet gates as unmet, not as a successful test run. |
...
The useful inference is about **observable design patterns associated with selected exemplars**, not causation. The source snapshots show that sharp activation language, executable evidence, narrow completion gates, and composable boundaries are present in those packages; they do not prove that any one feature produced adoption or a quality outcome.
...
- **Review all files, not only the description.** Socket documents malicious behavior hidden in referenced Python, shell, Markdown, and other files and notes that mutable GitHub sources require rescanning ([Socket](https://www.socket.dev/blog/socket-brings-supply-chain-security-to-skills)). The Gen Agent Trust Hub audit of `logo-creator` identifies shell-profile credential access and subprocess execution as high-risk behavior that the skill's purpose does not reveal. It also notes transmission of image data to external services, but that external transfer is part of the skill's stated purpose; it should not be conflated with the credential and process risks ([audit](https://www.skills.sh/resciencelab/opc-skills/logo-creator/security/agent-trust-hub)).
- **Audit-time finding: `mattpocock/skills/code-review`.** The public [Gen Agent Trust Hub audit](https://www.skills.sh/mattpocock/skills/code-review/security/agent-trust-hub) is dated 2026-08-20 and reports **Warn / Medium** for command execution and prompt injection; the [Snyk audit](https://www.skills.sh/mattpocock/skills/code-review/security/snyk), also dated 2026-08-20, reports **Fail / High**. Snyk's W007 finding says the skill's instruction to pass the full diff and commit list to sub-agents and quote the hunk or specification lines verbatim can reproduce secrets in agent output. The public audit pages do not expose a source or content hash tying these results to the later pinned commit `c55ee460`, so these are audit-time findings about the version reviewed by the auditors, not a verified result for the pinned commit. They are not a 2026-09-24 audit result, and they are not proof that the skill is inherently unsafe. Users should review the [pinned skill source](https://github.com/mattpocock/skills/blob/c55ee46073ed923f86ce59a5eb3b6d895095d1b7/skills/engineering/code-review/SKILL.md), treat repository content as untrusted, and redact or bound quoted material before adoption.
